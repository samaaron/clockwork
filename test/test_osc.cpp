...
//   /clockwork/schedule <ntp> </clockwork/osc/send <host> <port> <inner>>
...
static osc_test::Packet scheduleOscSend(double ntp, const char* host, int port,
    ...
    // int64 OSC timetag (the 'h' path clients use), not a double.
    ...
    sch.begin(CLOCKWORK_SYS("schedule"))
        << timetag
...
// ── The client half: osc/send ────────────────────────────────────────────────
// Every case from here down sends through /clockwork/osc/send, which is
// CLOCKWORK_CLIENT_VERBS (docs/SURFACE.md); a build without it refuses the
// verb by name, and test_client_verbs_absent.cpp pins that.
...
TEST_CASE("scheduled OSC out is delivered to the target host:port", "[osc]") {
    ...
    // /clockwork/schedule <ntp (just past → due now)> </clockwork/osc/send <host> <port> <inner>>.
...
// Self-loop: a client that targets its own cue port — the engine sends to
// its OWN cue port and must receive it back as a cue. "localhost"
...
TEST_CASE("OSC out: large message delivered, engine survives", "[osc]") {
    ...
    // A message the base 1 KB cell refuses and the 8 KB class carries, then
    // one only the 64 KB class carries. Each must arrive whole.
    auto receive = [&](const char* addr, size_t expectBytes) -> bool {
        ...
        while (std::chrono::steady_clock::now() < deadline) {
            if (listener.waitUntilReady(true, 50) == 1) {
                ...
                if (n > 0) {
                    CHECK(osc_test::parseReply(reinterpret_cast<uint8_t*>(buf), (uint32_t)n).address == addr);
                    CHECK(static_cast<size_t>(n) > expectBytes);
                    return true;
                }
            }
        }
    ...
    fx.waitForBlocks(5);
    REQUIRE(receive("/big", 4000));
    schedule(blobMsg("/bigger", 60000));
    fx.waitForBlocks(5);
    REQUIRE(receive("/bigger", 60000));

    // Wider than the widest class: refused, and the CALLER is told — the
    // refusal is /clockwork/error naming the verb and the reason, not a line
    // on a debug channel nobody is watching.
    fx.clearReplies();
    schedule(blobMsg("/toobig", 66000));
    OscReply err;
    REQUIRE(fx.waitForReply(CLOCKWORK_SYS("error"), err, 3000));
    auto ep = err.parsed();
    REQUIRE(ep.argCount() >= 2);
    CHECK(ep.argString(0) == CLOCKWORK_SYS("osc/send"));
    CHECK(ep.argString(1).find("exceeds") != std::string::npos);
    CHECK_FALSE(receive("/toobig", 0));
