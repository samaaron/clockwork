...
// =============================================================================
// chooseBlockSize — auto-match the engine block size to the HW callback
//
...
TEST_CASE("ChooseBlockSize: below floor falls back to default",
          "[ChooseBlockSize]") {
    // HW buffer smaller than the engine's minimum block size (32) — fall
...
TEST_CASE("ChooseBlockSize: above ceiling falls back to default",
          "[ChooseBlockSize]") {
    // Exotic HW buffer bigger than the engine's max — fall back.
...
TEST_CASE("FollowDefault: our own MyProduct aggregate elevated to default → ignore",
          "[FollowDefault]") {
    // Creating an aggregate can briefly make it the system default.
    REQUIRE_FALSE(follow("MyProduct#7", "MacBook Pro Speakers", false));
}
...
TEST_CASE("FollowDefault: self-aggregate detection follows the app name",
    ...
    // ignored — the literal "MyProduct" must not be baked in.
...
TEST_CASE("FollowDefault: real device sharing the app-name prefix is followed",
    ...
    REQUIRE(follow("MyProduct Audio Interface", "MacBook Pro Speakers", false,
                   "MyProduct"));
}
...
TEST_CASE("DeviceVisible: exact match present → visible (safe to open)",
          "[DeviceVisible]") {
    REQUIRE(visible("MyProduct#7", {"MacBook Pro Speakers", "MyProduct#7"}));
}

TEST_CASE("DeviceVisible: absent → not visible (keep polling, don't open)",
          "[DeviceVisible]") {
    REQUIRE_FALSE(visible("MyProduct#7", {"MacBook Pro Speakers"}));
}
...
TEST_CASE("DeviceVisible: empty list or empty name → not visible",
          "[DeviceVisible]") {
    REQUIRE_FALSE(visible("MyProduct#1", {}));
    REQUIRE_FALSE(visible("", {"MyProduct#1"}));
}
...
// parseHardwareFlag
// =============================================================================
// -H takes one or two device names: two-name reads as "<input> <output>" and
// a single name is mirrored into BOTH directions, so a client can pass all
// three shapes: -H <in> <out>,
...
TEST_CASE("FollowDefault: pinned output vetoes following", "[FollowDefault]") {
    ...
    REQUIRE_FALSE(shouldFollowDefaultOutputChange(
        "WH-1000XM5", "Testy", false, "MyProduct", "Testy"));
    ...
    REQUIRE_FALSE(shouldFollowDefaultOutputChange(
        "External Headphones", "MacBook Pro Speakers", false, "MyProduct",
        "Testy"));
}
