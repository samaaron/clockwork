...
// the gateway — see test_ring_reader.cpp) so /dummy/ping stops being answered. The
...
TEST_CASE("shm-peer crash: a corpse holding the writer lock stalls nobody; reattach recovers it",
    ...
    // Completeness + order: tempo.reply ids 1..kBurst, monotone, none missing.
    REQUIRE(fx.pollUntil([&] {
        int expected = 1;
        for (const auto& r : fx.allReplies()) {
            if (r.address != CLOCKWORK_SYS("clock/tempo.reply")) continue;
            if (lastInt(r) == expected) ++expected;
        }
