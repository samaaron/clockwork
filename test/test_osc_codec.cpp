...
/*
 * test_osc_codec.cpp — pins the test harness's own OSC codec (OscTestUtils.h).
 *
 * Every other test in this suite states its expectations in terms of these
 * helpers: it builds a packet with message()/Builder and reads the answer back
 * with parseAddress()/parseReply(). If the codec is wrong, every one of those
 * tests is wrong in the same direction and none of them notice. So the codec is
 * pinned first, on its own, by round-tripping each builder through the parser.
 *
 * Addresses here are arbitrary — they exist only to prove the encoder and the
 * parser agree about bytes, and name nothing in particular.
 */
...
TEST_CASE("parseAddress extracts correct address pattern", "[osc_codec]") {
    // The address is arbitrary and exists only to prove the mechanism carries
    // bytes — parseAddress must return whatever was encoded, whatever it names.
    auto pkt = osc_test::message("/test/filler", (int32_t)1000);
    auto addr = osc_test::parseAddress(pkt.ptr(), pkt.size());
    CHECK(addr == "/test/filler");
}
...
// ---------------------------------------------------------------------------
// 11. The wide numeric accessors round-trip through the Builder
// ---------------------------------------------------------------------------
TEST_CASE("Builder round-trips int64 and double args", "[osc_codec]") {
    const int64_t big = (int64_t{0x0BADF00D} << 32) | int64_t{0x12345678};
    osc_test::Builder b;
    auto& s = b.begin("/wide");
    s << static_cast<osc::int64>(big) << 1.0e-9;
    auto pkt = b.end();

    auto parsed = osc_test::parseReply(pkt.ptr(), pkt.size());
    CHECK(parsed.address == "/wide");
    CHECK(parsed.argCount() == 2);
    CHECK(parsed.argInt64(0) == big);          // full 64 bits, no float rounding
    CHECK(parsed.argDouble(1) == Catch::Approx(1.0e-9).margin(1e-18));
}
