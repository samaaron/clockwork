...
/*
 * test_ring_buffer_write.cpp — clockwork's OUT/egress ring writer.
 *
 * Contract pinned here: the wire format of a frame clockwork writes on the
 * audio thread, and the space arithmetic that decides whether it may be
 * written at all. Nothing below knows what a frame CONTAINS — the payload is
 * arbitrary bytes throughout — so this holds whichever DSP is attached.
 *
 * The OUT ring uses a contiguous-write design: a frame never wraps the buffer
 * boundary. When it does not fit before the end, a PADDING_MAGIC marker fills
 * the tail and the write restarts at 0. The space check must account for the
 * bytes wasted by padding, so a wrap-and-write can never overwrite unread
 * data.
 *
 * These exercise the REAL `ring_buffer_write` from audio_processor.cpp
 * (declared extern below). It takes its head/tail/sequence/status-flags as
 * parameters, so it can be driven with local atomics — no engine globals, no
 * arena, no device.
 */
...
#include "shared_memory.h"   // Message/MESSAGE_MAGIC/PADDING_MAGIC, EgressRoute, StatusFlags
...
TEST_CASE("ring_buffer_write frames header, payload and advancing sequence", "[RingBuffer]") {
    ...
    // Arbitrary bytes: the writer never inspects the payload, and this file
    // proves only that the transport carries them back unchanged.
    ...
    REQUIRE(route == EGRESS_BROADCAST_NOTIFY);  // route word precedes the payload
