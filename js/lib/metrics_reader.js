...
export class MetricsReader {
  ...
  gatherMetrics(context = {}) {
    ...
    const metrics = {
      // engine metrics
      engineProcessCount: m[MetricsOffsets.ENGINE_PROCESS_COUNT],
      engineMessagesProcessed: m[MetricsOffsets.ENGINE_MESSAGES_PROCESSED],
      engineMessagesDropped: m[MetricsOffsets.ENGINE_MESSAGES_DROPPED],
      engineSchedulerDepth: m[MetricsOffsets.ENGINE_SCHEDULER_DEPTH],
      engineSchedulerPeakDepth: m[MetricsOffsets.ENGINE_SCHEDULER_PEAK_DEPTH],
      engineSchedulerDropped: m[MetricsOffsets.ENGINE_SCHEDULER_DROPPED],
      engineSequenceGaps: m[MetricsOffsets.ENGINE_SEQUENCE_GAPS],
      engineSchedulerLates: m[MetricsOffsets.ENGINE_SCHEDULER_LATES],
      ...
      // Error metrics
      engineWasmErrors: m[MetricsOffsets.ENGINE_WASM_ERRORS],
    ...
    // Whatever the guest declared, read back out of the range it declared it
    // in. Clockwork names none of these.
    for (const [name, def] of Object.entries(this.#guestMetrics)) {
      metrics[name] = m[MetricsOffsets.GUEST_METRICS_BASE + def.slot];
    }
  ...
  updateMergedArray(context = {}) {
    ...
    // The guest's own numbers, by declared name into the declared slot. A
    // value for a name that was never declared has nowhere to go and is
    // dropped; a declared name with no value this tick reads zero.
    const guestValues = context.guestMetrics || {};
    for (const [name, def] of Object.entries(this.#guestMetrics)) {
      const v = guestValues[name];
      arr[MetricsOffsets.GUEST_METRICS_BASE + def.slot] = Number.isFinite(v) ? v : 0;
    }
