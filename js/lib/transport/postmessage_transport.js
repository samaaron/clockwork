 ...
 *
 * Architecture:
 * - OSC OUT: producers postMessage straight to the worklet, which classifies
 *            and schedules on the audio thread
...
export class PostMessageTransport extends Transport {
    ...
    async initialize(workletPort) {
        ...
        // Only now may the worklet drain the egress this way: a batch posted
        // before the handler above existed went to nobody.
        this.#workletPort.postMessage({ type: 'listening' });

        // Producers postMessage OSC straight to the worklet, which classifies
        // and schedules it on the audio thread.
    ...
    #handleWorkletMessage(data) {
        switch (data.type) {
            case 'oscReplies':
                // OSC replies from the engine (packed buffer format)
