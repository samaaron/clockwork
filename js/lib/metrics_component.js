 ...
 *
 * Renders metrics panels from Clockwork.getMetricsSchema().
...
class ClockworkMetrics extends HTMLElement {
  #sonic = null;
  #intervalId = null;
   ...
   * or can be called with a Clockwork class to show placeholder panels before boot.
   * @param {Function} ClockworkClass - The Clockwork class (for static getMetricsSchema)
   */
  buildFromSchema(ClockworkClass) {
    const schema = ClockworkClass.getMetricsSchema();
  ...
  /**
   * Connect to a Clockwork instance and start rendering.
   * @param {Object} sonic - Clockwork instance
...
if (typeof customElements !== 'undefined' && !customElements.get('clockwork-metrics')) {
  customElements.define('clockwork-metrics', ClockworkMetrics);
}

export { ClockworkMetrics };
