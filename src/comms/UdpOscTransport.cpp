...
/*
 * UdpOscTransport.cpp — see UdpOscTransport.h. Inbound via clockwork_osc_ingress (Rust
 * std::net), outbound via clockwork_osc_send; the token table + subscriber audiences are
 * portable C++. No JUCE.
 */
...
namespace {
// clockwork_osc_create needs an inbound emit callback for its cue server, which this
...
void UdpOscTransport::stop() {
    if (mIngress) { clockwork_osc_ingress_stop(mIngress); mIngress = nullptr; }
}
...
void UdpOscTransport::sendTo(const std::string& ip, int port,
    ...
    clockwork_osc_send(mOsc, reinterpret_cast<const uint8_t*>(ip.data()),
                static_cast<uint32_t>(ip.size()), port, data, size);
}

// ── IOscTransport: resolve a token / audience → (ip,port) → clockwork_osc_send ───────
