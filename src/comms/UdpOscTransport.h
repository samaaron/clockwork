 ...
 * received by the Rust std::net subsystem (clockwork_osc) and handed to the ingest
 * callback carrying an interned origin token; outbound replies/broadcasts go back
 * out through clockwork_osc_send. The token address-book (OriginTable) and the notify
 * subscriber audiences are the transport's portable address book; only the
 * actual sockets live in the Rust leaf.
 *
...
private:
    ...
    IngestFn      mIngest;
    ClockworkOsc*        mOsc     = nullptr;   // outbound send handle
    ClockworkOscIngress* mIngress = nullptr;   // inbound recv (owns the Rust recv threads)
