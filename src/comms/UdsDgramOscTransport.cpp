 ...
 * clockwork_osc_uds_dgram_start (Rust std::os::unix::net), outbound via
 * clockwork_osc_uds_dgram_send; the token table + subscriber audiences are portable
 * C++. No JUCE.
 */
...
bool UdsDgramOscTransport::start() {
    ...
    mServer = clockwork_osc_uds_dgram_start(
...
void UdsDgramOscTransport::stop() {
    if (mServer) { clockwork_osc_uds_stop(mServer); mServer = nullptr; }
}
...
bool UdsDgramOscTransport::sendTo(const std::string& path,
    ...
    return clockwork_osc_uds_dgram_send(mServer,
