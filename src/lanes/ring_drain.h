 ...
 * lanes egress drains (lanes.cpp) and the native RingReader thread. A browser
 * reader is this code too, reached through clockwork_client_poll and
 * clockwork_client_tap_poll; the separate JavaScript walk that used to shadow
 * it is gone.
 ...
 *
 * Single-consumer per ring: the caller owns the tail and the ClockworkDrainState.
 */
#pragma once
...
enum class ClockworkDrainVerdict : uint8_t { Consume, Retain };
...
enum class ClockworkDrainStop : uint8_t {
...
// Field order is part of the contract: callers brace-initialise.
struct ClockworkDrainMetrics {
...
inline uint32_t clockwork_drain_ring(uint8_t*              buffer,
                              uint32_t              size,
                              std::atomic<int32_t>* headPtr,
                              std::atomic<int32_t>* tailPtr,
                              ClockworkDrainState&         st,
                              const ClockworkDrainMetrics& metrics,
                              uint32_t              maxFrames,
                              OnMsg&&               onMessage,
                              ClockworkDrainStop*          stopReason = nullptr) {
    ClockworkDrainStop localStop = ClockworkDrainStop::Empty;
    ClockworkDrainStop& stop = stopReason ? *stopReason : localStop;
    stop = ClockworkDrainStop::Empty;
    ...
    while (maxFrames == 0 || consumed < maxFrames) {
        ...
        if (space_to_end < 4 || avail < 4) {
            if (metrics.corrupted) metrics.corrupted->fetch_add(1, std::memory_order_relaxed);
            tailPtr->store(head, std::memory_order_release);
            stop = ClockworkDrainStop::BadMagic;
            break;
        }
        ...
        if (space_to_end < sizeof(Message)) {
            ...
            if (metrics.corrupted) metrics.corrupted->fetch_add(1, std::memory_order_relaxed);
            tailPtr->store(head, std::memory_order_release);
            stop = ClockworkDrainStop::BadLength;
            break;
        }
        ...
        if (payloadSize > 0) {
            ClockworkDrainVerdict verdict = onMessage(hdr.sourceId, payload, payloadSize, hdr.sequence);
            if (verdict == ClockworkDrainVerdict::Retain) {
                ...
                stop = ClockworkDrainStop::Retained;
                break;
            }
        }
    ...
    if (stop == ClockworkDrainStop::Empty && maxFrames != 0 && consumed >= maxFrames)
        stop = ClockworkDrainStop::MaxFrames;
    return consumed;
}
