...
/*
 * DevicePolicy.h — the swap and hot-plug decisions, as pure functions so the
 * rules can be unit-tested without a real audio device. EACH ONE CORRESPONDS TO
 * A BUG THAT HAS BEEN FIXED AND SHOULD STAY FIXED.
 *
 * Internal: tests and ClockworkEngine.cpp include it; embedders see only
 * ClockworkEngine.h.
 */
#pragma once
...
// against the boot -i flag: an explicit boot count wins; otherwise the
// device's full width is requested (kRequestMaxChannels) — a boot with
// inputs disabled (-i 0) says nothing about how many the user wants once
// they turn inputs on, so it must not cap them at stereo (that cap left
// multichannel interfaces at 2 inputs after a live enable). Stereo is
// requested only when inputs were boot-disabled AND the probe failed:
// WASAPI rejects an over-request outright, so with no capacity known the
// safe number is the one every device has.
// The result — auto or explicit — is then clamped to `probedMax`, the
// device's probed input capacity (<= 0 = probe failed / unknown, no
// clamp). SHARED by switchDevice's auto-enable and enableInputChannels:
// resolving them separately shipped 64 input bits into WASAPI on the
...
//
// An unmatched name is returned unchanged, so JUCE errors normally rather than
// being silently rewritten. The match is strict on "<raw> (digits)": a
// prefix+space check would match "USB Audio Pro" against "USB Audio".
...
// a client's boot handshake. Policy: if the default is wireless, find
...
// instead of opening the platform default and being cold-swapped after the
// handshake. `hasDeviceRequest` is whether a -H output device came with it.
//
// Exact type name, then unique case-insensitive (ambiguity is no match).
// Anything unresolved keeps the platform default with a warning.
//
// "ASIO" WITH NO -H DEVICE IS REFUSED: ASIO has no default-device concept and
// probing one can hang — IASIO::init blocks in COM for registered-but-unplugged
...
// means "leave unchanged"; "__system__" (output) and "__none__" (input) are
// sentinels; anything else must appear in visibleDevices, bare or as "<name> (N)".
//
// switchDevice destroys the DSP, removes the audio callback and re-aims the
// channel ceilings before it ever reaches setAudioDeviceSetup. A name that only
// fails THERE leaves half-built state with no easy rollback.
...
//   3. inputs that failed against the current output are NOT removed — a list
//      that shrinks as the user tries things reads as the app losing hardware;
...
// Where a swap will land, driver-wise. Accumulated across the output and input
// names of one swap request by resolveSwapTarget; crossDriver latches first-wins
// — the first name resolving under a driver other than JUCE's actual type decides
// the transition, and the second never overwrites it.
struct SwapScope {
...
//      configuration (resolveInputWidth, probed clamp) — forces cold;
//   6. channel-count change at the target forces cold (a hot swap keeps
//      the DSP instance, whose channel counts are only re-read when it
//      is rebuilt);
...
struct SwapPlan {
    ...
    int  enableInputWidth = -1;          // >= 0: set the DSP input width, cold
...
// THE device-name identity predicate: equal, or equal modulo JUCE's " (N)"
// duplicate-disambiguation suffix (digits only), in either direction. NOT a
// "base + space" prefix rule — that matches "USB Audio Pro" against "USB Audio",
// letting the hotplug auto-switch treat a different physical device as the
// preferred one.
