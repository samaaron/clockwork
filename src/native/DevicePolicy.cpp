...
SwapPlan planSwap(const SwapPlanRequest& req, const SwapSnapshot& snap) {
    ...
    // Input auto-enable: a device was named against a zero-input configuration.
    ...
    // A hot swap keeps the existing DSP instance, which only re-reads its
    // channel counts when it is rebuilt — so a channel-count change at the
    // target forces cold. Without it the DSP keeps writing into the channel
    // range it booted with, and anything above that never reaches hardware.
...
int resolveInputWidth(int requested, int bootInputChannels, int probedMax) {
    int width = requested;
    if (requested < 0) {
        if (bootInputChannels > 0)
            width = bootInputChannels;
        else if (bootInputChannels == 0 && probedMax <= 0)
            width = 2;  // unknown capacity and no boot count: the one safe number
        else
            width = kRequestMaxChannels;
    }
...
DeviceSwitchPlan planDeviceSwitch(
    ...
    // driver transitions come from /clockwork/drivers/switch, whose
    // recorded intent resolveSwapScope then scopes against — never
    // an implicit side effect of a device pick.
