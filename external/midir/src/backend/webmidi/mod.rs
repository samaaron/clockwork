...
impl MidiOutputConnection {
    ...
    /// Schedule `message` for delivery `delay` from now.
    ///
    /// Web MIDI wants an absolute `DOMHighResTimeStamp` in the same coordinate
    /// as `performance.now()`, so the delay is added to it here. The browser's
    /// MIDI subsystem does the scheduling, which is what makes look-ahead the
    /// right approach on the web: handing it a future timestamp is TIGHTER than
    /// racing to deliver on time across a thread boundary.
    ///
    /// A timestamp in the past sends immediately, per the Web MIDI spec.
    ///
    /// NOTE for tau: this backend is NOT in our web build. `midir` is a
    /// `cfg(not(target_arch = "wasm32"))` dependency, and on the web outgoing
    /// MIDI is sent from the main thread by js/lib/midi_manager.js, which calls
    /// `MIDIOutput.send(bytes, timestamp)` itself — the Web MIDI API lives on
    /// the main thread, so there is nothing for Rust to add. This exists to
    /// keep midir's own API uniform across backends, not because we use it.
    pub fn send_at(&mut self, message: &[u8], delay: Duration) -> Result<(), SendError> {
        let at = now_ms() + delay.as_secs_f64() * 1000.0;
        self.output
            .send_with_timestamp(unsafe { Uint8Array::view(message) }.as_ref(), at)
            .map_err(|_| SendError::Other("JavaScript exception"))
    }

    /// True: the browser schedules.
    pub fn schedules(&self) -> bool {
        true
    }
}

/// `performance.now()`, read off the global rather than off `window`, so this
/// works in a worker as well as a document.
///
/// 0.0 if it cannot be read, which puts the timestamp in the past and so sends
/// immediately — the same behaviour as having no scheduling at all, rather
/// than a message lost or sent at the wrong time.
fn now_ms() -> f64 {
    ...
    match now.dyn_into::<js_sys::Function>() {
        Ok(f) => f.call0(&perf).ok().and_then(|v| v.as_f64()).unwrap_or(0.0),
        Err(_) => 0.0,
    }
}
