...
//! nominal 192, queues at 32), so an application that opens *every* port can
//! exhaust them and make later `snd_seq_open` calls from other programs (e.g.
//! `aplaymidi`) fail with "Cannot allocate memory".
//!
...
/// (clockwork passes the normalised handle and the raw OS name).
...
pub struct SharedOutput {
    ...
    /// One queue for the whole client, for `send_at`. Queues are client-scoped
    /// in ALSA, so one here covers every port this client owns. -1 if the
    /// sequencer would not allocate one, in which case `send_at` refuses.
    queue_id: i32,
}
...
impl SharedOutput {
    ...
    pub fn new(client_name: &str) -> Result<Self, InitError> {
        ...
        let cname = CString::new(client_name).map_err(|_| InitError)?;
        seq.set_client_name(&cname).map_err(|_| InitError)?;
        let queue_id = super::init_output_queue(&seq);
        Ok(SharedOutput {
            seq,
    ...
    /// Send to one label, `delay` from now, on this client's queue.
    ///
    /// Scheduled RELATIVE, so nothing has to map our clock onto the queue's:
    /// the kernel holds the event for `delay` after it receives it and delivers
    /// it itself. That is the whole point — a thread of ours waking up to send
    /// on time collects its own scheduling jitter, and a MIDI clock is where
    /// that shows.
    pub fn send_at(&mut self, label: &str, message: &[u8], delay: std::time::Duration) -> bool {
        if self.queue_id < 0 {
            return false;
        }
        let vport = match self.ports.get(label) {
        ...
        // Queued, not direct: direct is what bypasses the queue.
        if self.seq.event_output(&mut ev).is_err() {
            return false;
        }
        let _ = self.seq.drain_output();
        true
    ...
    /// Whether this client got a queue and can therefore schedule.
