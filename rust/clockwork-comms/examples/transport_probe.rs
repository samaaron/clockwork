...
//!   transport_probe load <proto> <target> <count> load: blast <count> /clockwork/sync ids,
//!                                                 verify the /clockwork/synced replies
//!
...
fn sync_msg(id: i32) -> Vec<u8> {
    encode("/clockwork/sync", &[OscArg::Int(id)])
}

/// The id of a `/clockwork/synced` reply, if that's what the packet is.
fn synced_id(pkt: &[u8]) -> Option<i32> {
    let m = decode(pkt)?;
    if m.addr != "/clockwork/synced" {
        return None;
    }
...
/// Blast `count` /clockwork/sync ids down a reliable stream while a reader thread collects
/// the /clockwork/synced replies; every id must come back exactly once, in order.
...
/// Blast `count` /clockwork/sync ids over a datagram socket, then drain replies until they
...
fn load(proto: &str, target: &str, count: i32) -> Result<String, String> {
    match proto {
        ...
        "shm" => {
            ...
            // a real peer would — so nothing is dropped and /clockwork/synced ids arrive in
