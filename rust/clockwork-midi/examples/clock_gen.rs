...
//!
//! Creates a virtual MIDI output port ("Test Clock") and streams 24-PPQN
//! `0xF8` timing-clock bytes at a BPM you change live from the keyboard. Any app
//! (clockwork included) that enables this port as MIDI input sees a
//! `midi:test_clock` timeline whose tempo follows what you type here.
//!
//! Run (macOS / Linux — virtual ports aren't supported on Windows midir):
//!     cargo run -p clockwork-midi --example clock_gen
//!
//! Then at the prompt:
...
#[cfg(unix)]
const PORT_NAME: &str = "Test Clock";
...
fn main() {
    ...
    #[cfg(unix)]
    {
        ...
        println!("Enable it as MIDI input in the engine and follow the midi:test_clock timeline.");
