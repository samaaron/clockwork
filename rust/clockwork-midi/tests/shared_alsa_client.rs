...
//!
//! Regression test: the old midir path opened *one ALSA
...
/// What arrived, by port name, shared with the receiving callback.
...
fn one_client_per_side_regardless_of_port_count() {
    let tag = "ss3543";
    let client_name = "clockwork-shared-3543";
    ...
    let got: Received = Arc::new(Mutex::new(Vec::new()));
    let sink = got.clone();
    ...
    // At most one queue per SIDE, not one per port. Two, since 2026-08-30:
    // the input client has always had one (to timestamp incoming events) and
    // the output client gained one for `send_at`, which is what lets the kernel
    // hold a scheduled message instead of a thread of ours.
    //
    // The bound that matters is not the constant but that it IS a constant:
    // >=3 ports are open on each side above, so a regression to one queue per
    // port would show at least 6 here, not 3.
    let idset: HashSet<i32> = ids.iter().copied().collect();
    let qn = queues_owned_by(&idset);
    assert!(
        qn <= 2,
        "expected <=2 ALSA queues owned by our clients (one per side), found \
         {qn} with {} inputs and {} outputs open — one queue per port regressed",
        open_in.len(),
        open_out.len()
    );
}
...
fn input_from_multiple_ports_routes_with_correct_labels() {
    let tag = "ss3543b";
    let client_name = "clockwork-shared-3543b";
