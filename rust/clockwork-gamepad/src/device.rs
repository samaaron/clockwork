...
//! `clockwork_osc::normalize`), assigned at connect time and stable for the
...
impl GamepadIo {
    /// The error is boxed: `gilrs::Error` is a large enum, and the one
    /// caller only checks for it.
    pub fn new(registry: Arc<Mutex<Registry>>) -> Result<Self, Box<gilrs::Error>> {
        let gilrs = Gilrs::new()?;
    ...
    /// of spinning. Events from pads muted via `/clockwork/gamepad/enable … 0` still
