...
//! hands back, so the `/clockwork/gamepad/in/*` stream is identical on web and native:
...
/// every shell (native backends via `Out::from_pad_event`, the wasm boundary)
/// names elements through this, so the `/clockwork/gamepad/in/*` vocabulary cannot
