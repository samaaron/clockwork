...
// MIDI ports and web-side pads. See clockwork_osc::normalize.
pub use crate::normalize::assign_handle;

/// The (handle, enabled) registry shared with the FFI shell, which reads it to
/// answer `/clockwork/gamepad/devices/list` synchronously on the caller's thread and
/// writes it for `/clockwork/gamepad/enable`. Rows are in connect order; the backends
...
pub struct Registry {
    ...
    /// Applied to pads that connect later (set by `/clockwork/gamepad/enable * …`).
