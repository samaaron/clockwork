...
//! the identifier clients use in `/clockwork/midi/*` / `/clockwork/gamepad/*` messages, so it must
...
/// Characters that are unsafe in an OSC address (or reserved by a client's
/// `:`-delimited names) are replaced with `_`.
const UNSAFE: &[char] = &[' ', '#', '*', ',', '/', '?', '[', ']', '{', '}', ':'];
...
/// addressed by `/clockwork/midi/*`, and their per-port clock timelines merge).
pub fn normalize_ports(raw_names: &[String]) -> Vec<PortInfo> {
    let mut out: Vec<PortInfo> = Vec::with_capacity(raw_names.len());
    let mut used: alloc::collections::BTreeSet<String> = alloc::collections::BTreeSet::new();
