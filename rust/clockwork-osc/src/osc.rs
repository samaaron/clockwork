...
//!
//! `encode`/`decode` operate on a flat message (what the `/clockwork/midi/*`, `/clockwork/gamepad/*`
//! and `/clockwork/osc/*` schemas use day to day). `encode_packet`/`decode_packet` add the
...
pub enum OscArg {
    ...
    Symbol(String),   // 'S'
    Blob(Vec<u8>),    // 'b'
    /// Clockwork extension: a 128-bit uuid as (hi, lo), two big-endian u64s on
    /// the wire. Not OSC 1.0; both ends of the clockwork API speak it.
    Uuid(u64, u64),   // 'u' — 16 bytes
    Array(Vec<OscArg>), // '[' .. ']'
}
...
fn read_arg<'a>(
    ...
    Some(match t {
        ...
        'u' => {
            ...
            let hi = u64::from_be_bytes(rest[..8].try_into().ok()?);
            let lo = u64::from_be_bytes(rest[8..16].try_into().ok()?);
            (OscArg::Uuid(hi, lo), &rest[16..])
        }
        '[' => {
            if depth >= MAX_DEPTH {
                return None;
...
mod tests {
    ...
    fn roundtrips_mixed_args() {
        ...
        let bytes = encode(clockwork_sys!("midi/in/note_on"), &args);
        assert_eq!(bytes.len() % 4, 0);
        let msg = decode(&bytes).unwrap();
        assert_eq!(msg.addr, clockwork_sys!("midi/in/note_on"));
    ...
    #[test]
    fn empty_args() {
        let bytes = encode(clockwork_sys!("midi/refresh"), &[]);
        let msg = decode(&bytes).unwrap();
        assert_eq!(msg.addr, clockwork_sys!("midi/refresh"));
    ...
    /// Miri interprets every instruction, so the loops below — which run in
    /// well under a second natively — take hours under it. They are scaled
    /// down there rather than skipped: what Miri is being asked for is the
    /// code path, and that is covered by the first few hundred iterations.
    /// The full counts still run on every ordinary `cargo test`.
    const fn scaled(full: u64, under_miri: u64) -> u64 {
...
mod uuid_wire_tests {
    ...
    #[test]
    fn a_uuid_round_trips_through_the_codec() {
        let args = [
            OscArg::Uuid(0x0123_4567_89ab_cdef, 0xfedc_ba98_7654_3210),
            OscArg::Int(7),
        ];
        let bytes = encode("/run/start", &args);
        let msg = decode(&bytes).expect("decodes");
        assert_eq!(msg.addr, "/run/start");
        assert_eq!(msg.args, args);
    }

    #[test]
    fn the_wire_form_is_sixteen_big_endian_bytes_tagged_u() {
        let bytes = encode("/x", &[OscArg::Uuid(1, 2)]);
        // "/x\0\0" + ",u\0\0" + 16 bytes
        assert_eq!(bytes.len(), 4 + 4 + 16);
        assert_eq!(&bytes[4..6], b",u");
        assert_eq!(&bytes[8..16], &1u64.to_be_bytes());
        assert_eq!(&bytes[16..24], &2u64.to_be_bytes());
    }

    #[test]
    fn a_truncated_uuid_is_refused_not_misread() {
        let mut bytes = encode("/x", &[OscArg::Uuid(1, 2)]);
        bytes.truncate(bytes.len() - 4);
        assert!(decode(&bytes).is_none());
    }
}

/// Now, as an OSC 32.32 fixed-point timetag.
///
/// Here because this crate owns the timetag type and is the one thing both the
/// MIDI and the sink crates already depend on — putting it in either would
/// make the other depend on it, and clockwork-sinks already depends on clockwork-midi for
/// its endpoint, so that edge cannot run the other way.
///
/// The SAME formula as `clockwork::ntpToOscTimetag(wallClockNTP())` in
/// src/clock/clock_math.h and `clockwork_sinks::time::now()`. It is spelled more than once
/// because a C++ header, a no_std-capable crate and a std crate cannot share
/// one definition; the tests assert the spellings agree, which is the same
...
/// How long from now until `when`, or `None` if it is already due.
///
/// Timetags are compared as UNSIGNED. A present-day timetag has its top bit
/// set, so as an i64 it is negative and every signed comparison gets the answer
/// exactly backwards — a trap this tree has hit three times in different files.
#[cfg(feature = "std")]
pub fn delay_until(when: u64) -> Option<std::time::Duration> {
    // 0 and 1 are the OSC "immediately" sentinels: not a moment, so nothing to
    // wait for.
    if when <= 1 {
        return None;
    }
    ...
    let diff = when - now;
    let secs = diff >> 32;
    let nanos = (((diff & 0xFFFF_FFFF) * 1_000_000_000) >> 32) as u32;
...
mod timetag_tests {
    ...
    fn an_immediate_sentinel_is_never_a_delay() {
        assert!(delay_until(0).is_none());
        assert!(delay_until(1).is_none());
    }

    #[test]
    fn a_past_timetag_is_due_not_negative() {
        // The signed-comparison trap: a real timetag is negative as an i64, so
        // a signed `when <= now` answers the opposite of the truth.
        let now = now_timetag();
        assert!(now > (i64::MAX as u64), "a present-day timetag has its top bit set");
        assert!(delay_until(now - (1u64 << 32)).is_none(), "a second ago is due");
    }
